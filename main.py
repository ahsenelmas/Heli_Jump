...
def generate_new():
    ...
    for i in range(total_rects):
        ...
        if top_height < 0:
            ...
            top_height = 300
        top_rect = pygame.draw.rect(screen, 'blue', [i * rect_width, 0, rect_width, top_height])
        bot_rect = pygame.draw.rect(screen, 'blue', [i * rect_width, top_height + 300, rect_width, HEIGHT])
...
def draw_map(rects):
    for i in range(len(rects)):
        pygame.draw.rect(screen, 'blue', rects[i])
...
while run:
    ...
    screen.blit(font.render(f'High Score: {high_score}', True, 'white'), (20, 565))
    if not active:
        screen.blit(font.render('Press Enter to Restart', True, 'white'), (300, 15))
        screen.blit(font.render('Press Enter to Restart', True, 'white'), (300, 565))
    pygame.display.flip()
pygame.quit()